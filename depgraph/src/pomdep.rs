...
impl PomGraph {
    ...
    /// This function is used to add more details above the existing graph generated by ferstl/depgraph
    pub fn to_dot<W: Write>(&self, out: &mut W) {
        write!(out, "digraph \"{}\" {{\n", &self.graph_name).unwrap();
        let indent: usize = 2;
        write!(out, "{:spaces$}node [shape=\"box\",style=\"rounded\",fontname=\"Helvetica\",fontsize=\"14\"]\n", "", spaces=indent).unwrap();
        write!(out, "{:spaces$}edge [fontsize=\"10\",fontname=\"Helvetica\"]\n", "", spaces=indent).unwrap();
        ...
        for n in &self.artifacts {
            ...
            write!(out, "{:spaces$}\"{}\"[label=<<font point-size=\"10\">{}</font><br/>{}<font point-size=\"10\"><br/>{}</font>{}>]\n",
                   "", n.id(), coord.group_id(), coord.artifact_id(), coord.version_id(), scope_line, spaces=indent).unwrap();
        }
        ...
        for e in &self.dependencies {
            ...
            write!(out, "{:spaces$}\"{}\" -> \"{}\"{}\n",
                   "", e.from(), e.to(), attr_str, spaces=indent).unwrap();
        }
        write!(out, "}}\n").unwrap();
    }
}
