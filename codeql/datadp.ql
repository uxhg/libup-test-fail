...
bindingset[j]
predicate notJavaLib(string j) {
  not j.regexpMatch("^java.*$")
}
...
class DataDepLibCalls extends TaintTracking::Configuration {
  ...
  where lib1 = source.getCallee().getDeclaringType().getQualifiedName() 
  and lib2 = sink.getCallee().getDeclaringType().getQualifiedName()
  and notSameJar(lib1, lib2)
  and e = sink.getAnArgument()
  and pt.hasFlow(DataFlow::exprNode(source), DataFlow::exprNode(e))
