...
**** Files in stacktrace
[[file:../../libup-cases/jersey-new/core-common/src/main/java/org/glassfish/jersey/model/internal/CommonConfig.java][jersey-common: model.internal.CommonConfig]]

[[file:../../libup-cases/jersey-old/media/multipart/src/main/java/org/glassfish/jersey/media/multipart/internal/FormDataParamValueParamProvider.java][jersey-media: multipart.internal.FromDataParamValueParamProvider]]

(new version, not in stacktrace) [[file:../../libup-cases/jersey-new/media/multipart/src/main/java/org/glassfish/jersey/media/multipart/internal/FormDataParamValueParamProvider.java][jersey-media: multipart.internal.FromDataParamValueParamProvider]]
**** Other related files

[[file:../../libup-cases/jersey-new/core-common/src/main/java/org/glassfish/jersey/model/Parameter.java][jersey-common:model.Parameter]]

*** Root Cause
    Since =jersey-server= relies on =jersey-common=, upgrading =jersey-server= to 2.29.1 will bring
    in =jersey-common:2.29.1=. While both =jersey-common:2.29.1= and =jersey-common:2.27= are on the
    dependency tree, =2.29.1= comes first.

    In the stack trace above, =jersey-media-multipart:2.27= still stays at the old version and there
    is a reference from =multipart.internal.FormDataParamValueParamProvider= to
    =server.model.Parameter.Source=.

    Class (actually an enum) =model.Parameter$Source= exists in =jersey-common:2.29.1=, but notice
    that the class not found is =server.model.Parameter.Source= in =jersey-server=.
    #+begin_src
public class Parameter implements AnnotatedElement {
    ...
    In the old version of =jersey-server=:
    =public enum Source= is defined inside =server.model.Parameter.Source=.

    In the new version of =jersey-server=, it inherits from the =model.Parameter= in =jersey-common=.
    #+begin_src java
